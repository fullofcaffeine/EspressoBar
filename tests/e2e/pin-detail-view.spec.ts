...
test.describe('Pin Detail View End-to-End', () => {
  test('should open detail view when command-clicking a pin item', async () => {
    ...
    await page.locator('[data-testid="pin-item"]').first().click({ modifiers: [modifier] })
  ...
  test('should close detail view when clicking close button', async () => {
    ...
    if (await page.locator('[data-testid="pin-detail-modal"]').isVisible()) {
      ...
      await expect(page.locator('[data-testid="pin-detail-modal"]')).not.toBeVisible({ timeout: 3000 })
    }
    ...
    // Verify modal closes
    await expect(page.locator('[data-testid="pin-detail-modal"]')).not.toBeVisible({ timeout: 5000 })
  ...
  test('should show "Open in Emacs" button for org file pins', async () => {
    ...
    await page.evaluate(async () => {
      ...
      if ((window as any).electronAPI && (window as any).electronAPI.addPin) {
        ...
        window.dispatchEvent(new CustomEvent('test-pins-updated', { 
          detail: mockStore 
        }))
      }
    })
    ...
    const testPinLocator = page.locator('[data-testid="pin-item"]', { hasText: 'Test org headline' })
    ...
    await expect(page.locator('[data-testid="pin-detail-modal"]')).toContainText('/Users/test/org-files/test.org')
    ...
    await page.evaluate(() => {
      ...
      if ((window as any).electronAPI) {
        ...
        ;(window as any).electronAPI.openInEmacs = async (filePath: string, lineNumber?: number) => {
  ...
  test('REGRESSION: org file pins should have filePath and show "Open in Emacs" button', async () => {
    console.log('🧪 REGRESSION TEST: Verifying org file pins have proper metadata and "Open in Emacs" button...')
    ...
    console.log('📄 Org file pins:', orgFilePins.map((p: any) => ({ 
      id: p.id, 
      content: p.content, 
      filePath: p.filePath, 
    ...
    for (const pinItem of pinItems) {
      ...
      if (text && (text.includes('Important task') || text.includes('Meeting notes') || text.includes('Project planning'))) {
        targetPin = pinItem
        break
      }
    }
    ...
    if (targetPin) {
      ...
      const hasSourceSection = await page.locator('[data-testid="pin-detail-modal"]', { hasText: 'Source' }).isVisible()
      ...
      if (hasSourceSection) {
        ...
        throw new Error('REGRESSION: Org file pins should have filePath and lineNumber metadata, but Source section is missing')
      }
    }
    
    console.log('✅ REGRESSION TEST PASSED: Org file pins have proper metadata and show "Open in Emacs" button')
  })
}) 
